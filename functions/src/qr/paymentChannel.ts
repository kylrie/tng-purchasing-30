...
export const PAYMENT_METHOD_CHANNEL: Readonly<Record<string, string>> = {
    gcash: 'GCASH',
    maya: 'PAYMAYA',
    qrph: 'QRPH',
    card: 'CARDS',
...
export type ChannelResolution =
    | { kind: 'all' }                              // no/blank method → unrestricted (all channels)
    | { kind: 'restricted'; channels: string[] }   // valid token → single-channel allowlist
    | { kind: 'invalid' };                         // non-blank unknown → reject (server authority)
 ...
 */
export function resolveAllowedPaymentChannels(method: unknown): ChannelResolution {
    if (method === undefined || method === null) return { kind: 'all' };
    if (typeof method !== 'string') return { kind: 'invalid' };
    const key = method.trim().toLowerCase();
    if (key === '') return { kind: 'all' };
    const code = PAYMENT_METHOD_CHANNEL[key];
