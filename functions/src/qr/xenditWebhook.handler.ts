...
/** The release service, injected (bound to the real db in the wrapper). */
export type ReleaseFn = (orderId: string, options: ReleaseOrderOptions) => Promise<ReleaseOrderResult>;

export interface WebhookConfig {
    /** XENDIT_CALLBACK_TOKEN — compared against the x-callback-token header. */
...
/** Ledger `result` values (Master Plan §6.3, plus 'ignored' for non-actionable). */
type LedgerResult = 'applied' | 'duplicate' | 'rejected' | 'ignored';
...
interface NormalizedEvent {
    ...
    sessionId: string;
    status: string;
    amount: number;      // NaN if absent/unparseable
    currency: string;
    channelCode: string;
    paymentMethodType: string;
}

type EventKind = 'paid' | 'failed' | 'expired' | 'other';
...
export function parseOrderId(referenceId: string): string | null {
...
export function classifyEvent(event: string, status: string): EventKind {
    const e = (event ?? '').toLowerCase();
    const s = (status ?? '').toUpperCase();
    if (e.includes('succeeded') || e.includes('completed') || s === 'SUCCEEDED' || s === 'COMPLETED' || s === 'PAID') return 'paid';
...
export function extractEvent(body: unknown): NormalizedEvent {
    const root = (body && typeof body === 'object') ? body as Record<string, unknown> : {};
    const data = (root.data && typeof root.data === 'object') ? root.data as Record<string, unknown> : root;
    const str = (v: unknown): string => (typeof v === 'string' ? v : '');
    const pm = (data.payment_method && typeof data.payment_method === 'object') ? data.payment_method as Record<string, unknown> : {};
    return {
...
export async function xenditWebhookHandler(
    ...
    const paidButUnreleased = (order: Record<string, unknown> | undefined): boolean =>
        !!order && (order.paymentStatus === 'PAID' || order.status === 'PAID') && order.released !== true;

    const outcome = await db.runTransaction(async (txn: Transaction) => {
        ...
        if (ledgerSnap.exists) {
            const dupOrder = orderRef ? (await txn.get(orderRef)).data() as Record<string, unknown> | undefined : undefined;
            return { result: 'duplicate' as LedgerResult, needsRelease: paidButUnreleased(dupOrder), orderId };
        }
        ...
        let result: LedgerResult = 'ignored';
        ...
        txn.set(ledgerRef, {
            ...
            businessUnitId: (order?.businessUnitId as string | undefined) ?? null,
    ...
    if (outcome.needsRelease && outcome.orderId) {
        try {
            ...
            console.error('xenditWebhook.release.error', { orderId: outcome.orderId, error: (e as Error).message });
        }
    }
