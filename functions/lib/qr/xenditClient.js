 ...
 */
class XenditClientError extends Error {
    retriable;
    status;
    constructor(message, retriable, status) {
        super(message);
...
exports.XenditClientError = XenditClientError;
function toMillis(expiresAt, fallback) {
 ...
 */
function createXenditHttpClient(config) {
    ...
    async function createSession(params) {
        ...
        try {
            ...
            console.error('xendit.createSession.transport', { referenceId: params.referenceId, error: e.message });
            throw new XenditClientError(`Xendit request failed: ${e.message}`, true);
        }
        finally {
            clearTimeout(timer);
        }
        if (!res.ok) {
            ...
            let bodyText = '';
            try {
                bodyText = (await res.text()).slice(0, MAX_LOGGED_BODY);
            }
            catch { /* ignore */ }
        ...
        try {
            ...
            console.error('xendit.createSession.parse', { referenceId: params.referenceId, error: e.message });
 ...
 */
function createMockXenditClient(now = Date.now) {
    return {
