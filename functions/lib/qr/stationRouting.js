...
exports.classifyStation = classifyStation;
exports.splitByStation = splitByStation;
...
const FALLBACK_SUBCATEGORY = { Food: 'Mains', Drinks: 'Soft Drinks' };
/**
...
function buildCategoryGroupIndex() {
    const index = new Map();
    for (const group of GROUPS) {
        for (const sub of group.subcategories) {
            if (!index.has(sub))
                index.set(sub, group.key);
        }
    }
    return index;
}
...
function normalizeCategory(raw) {
    ...
    if (exact)
        return { category: trimmed, group: exact };
    const aliased = CATEGORY_ALIAS[trimmed.toLowerCase()];
    if (aliased)
        return { category: aliased, group: INDEX.get(aliased) ?? DEFAULT_GROUP };
    const group = DRINK_CATEGORY_HINTS.some(h => trimmed.toLowerCase().includes(h)) ? 'Drinks' : DEFAULT_GROUP;
...
function refineByName(name, base) {
    const n = name ?? '';
    for (const rule of NAME_RULES) {
        if (rule.re.test(n))
            return { category: rule.category, group: INDEX.get(rule.category) ?? base.group };
    }
    return base;
}
/**
 ...
 */
function splitByStation(lines) {
    const out = { KITCHEN: [], BAR: [] };
    for (const line of Array.isArray(lines) ? lines : []) {
        out[classifyStation(line)].push(line);
    }
    return out;
}
//# sourceMappingURL=stationRouting.js.map