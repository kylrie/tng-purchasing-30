...
exports.checkoutOrder = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to create a POS order.');
    }
    const orderInput = request.data;
    if (!orderInput || !orderInput.businessUnitId || !orderInput.items || !Array.isArray(orderInput.items)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid order payload.');
    }
    ...
    const scRate = (settings?.serviceChargeRate || 0) / 100;
    // 2. Fetch current prices for all items in the cart
    const itemIds = orderInput.items.map((item) => item.menuItemId);
    ...
    let finalSubtotal = 0;
    const computedItems = orderInput.items.map((clientItem) => {
        const serverItem = menuItemsData[clientItem.menuItemId];
        if (!serverItem) {
            throw new https_1.HttpsError('not-found', `Menu item ${clientItem.menuItemId} not found.`);
        }
    ...
    return {
        ...
        order: {
            ...
            createdAt: firestore_1.Timestamp.now().toDate().toISOString(),
