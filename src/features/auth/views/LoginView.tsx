...
import { ShoppingCart, Mail, Lock, Sparkles, ArrowRight, Eye, EyeOff, UserCheck, ShieldAlert } from 'lucide-react';
...
const RegistrationModal = ({ onRegister, loading }: { onRegister: (role: UserRole) => void, loading: boolean }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 max-w-sm w-full relative">
        ...
        <p className="text-slate-400 mb-6">Welcome! Since this is your first time, please select your role.</p>
...
const LoginView = () => {
  const { loginWithEmail, loginWithGoogle, completeNewUserRegistration, mockLogin, isNewUser, loading, error, setError } = useAuth();
  ...
  const handleRegistrationSubmit = async (role: UserRole) => {
    await completeNewUserRegistration(role);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
      {isNewUser && <RegistrationModal onRegister={handleRegistrationSubmit} loading={loading} />}
      ...
      <div className="relative max-w-md w-full">
        ...
        <div className="relative bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-700/50 overflow-hidden">
          <div className="p-8 text-center relative">
            ...
            <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-400 via-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2">ProcureFlow</h1>
            <p className="text-slate-400 flex items-center justify-center gap-2"><Sparkles className="w-4 h-4" /> Next-Gen Purchasing System</p>
          </div>
          ...
          <div className="p-8 pt-0">
            ...
            <div className="mt-4 flex justify-center">
                 <button onClick={handleDevLogin} disabled={devLoading} className="text-xs text-purple-400 hover:text-purple-300 underline flex items-center gap-1">
                    <ShieldAlert className="w-3 h-3" />
                    {devLoading ? 'Logging in...' : 'Dev Login (Mock)'}
                 </button>
            </div>
