...
const SupplierModal: React.FC<SupplierModalProps> = ({ supplier, isOpen, onClose, onSave, businesses, currentUser }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
            <Card className="w-full max-w-3xl animate-in zoom-in-95 duration-200 !p-0 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto flex flex-col shadow-xl">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    {errorMessage && (
                        <div className="p-3 bg-red-50 dark:bg-red-900/50 border border-red-200 dark:border-red-500 rounded text-red-600 dark:text-red-200 text-sm">
                            {errorMessage}
                        </div>
                    )}
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                required
                                className="w-full p-2 bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none placeholder-slate-400 dark:placeholder-slate-500"
                                value={formData.name}
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                className={`w-full p-2 bg-white dark:bg-slate-900/50 border rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:outline-none placeholder-slate-400 dark:placeholder-slate-500 ${!isTinValid && formData.tin ? 'border-red-500 focus:ring-red-500' : 'border-slate-300 dark:border-slate-600 focus:ring-purple-500'}`}
                                value={formData.tin}
                                onChange={e => {
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Terms</label>
                            <input
                                className="w-full p-2 bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none placeholder-slate-400 dark:placeholder-slate-500"
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Assigned Business Units</label>
                        <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-lg p-2 h-32 overflow-y-auto custom-scrollbar">
                            {businesses.map(b => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`flex items-center gap-2 p-2 rounded cursor-pointer transition-colors ${isSelected ? 'bg-purple-100 dark:bg-purple-600/30 text-purple-700 dark:text-purple-200' : 'hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'}`}
                                    >
                                        <div className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? 'bg-purple-600 border-purple-600' : 'border-slate-400 dark:border-slate-500'}`}>
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        <div className="flex items-center justify-between mb-4">
                            <h4 className="text-md font-semibold text-slate-900 dark:text-white">Bank Details</h4>
                            <div className="flex items-center gap-3">
                                ...
                                <label htmlFor="hasBankDetails" className="text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                        ...
                        {formData.hasBankDetails && (
                            <div className="grid grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Branch (Optional)</label>
                                    <input
                                        className="w-full p-2 bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none placeholder-slate-400 dark:placeholder-slate-500"
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white rounded-lg font-medium transition-colors">
                            Cancel
                        </button>
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-300">Manage vendor relationships</p>
                </div>
                <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto items-stretch md:items-center">
                    <div className="relative flex-1 md:flex-none">
                        ...
                        <input
                            className="pl-10 p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm w-full md:w-64 text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none shadow-sm dark:shadow-none"
            ...
            <Card className="overflow-hidden !p-0 border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none bg-white dark:bg-slate-800">
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {filteredSuppliers.map(supplier => (
                                <tr key={supplier.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-200">{supplier.name}</td>
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {supplier.businessUnitIds && supplier.businessUnitIds.length > 0 ? (
                                                supplier.businessUnitIds.map(id => {
                                                    ...
                                                    return biz ? (
                                                        <span key={id} className="text-xs bg-purple-100 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-700/50 px-2 py-1 rounded text-purple-700 dark:text-purple-300 flex items-center gap-1">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1 text-amber-500 dark:text-amber-400">
                                            ...
                                            <span className="text-slate-600 dark:text-slate-300 font-medium">{supplier.rating}</span>
                                        </div>
                                    </td>
