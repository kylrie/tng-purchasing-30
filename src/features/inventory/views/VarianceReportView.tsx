...
const VarianceReportView: React.FC<VarianceReportViewProps> = ({ businesses }) => {
    ...
    const getVarianceTextClass = (variance: number) => {
        if (variance < -0.01) return 'text-red-400 font-semibold';
        if (variance > 0.01) return 'text-green-400 font-semibold';
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {/* Start Session */}
                    <div>
                        <label className="block text-sm text-slate-400 mb-2">Start Session (Earlier)</label>
                        <select
                            value={startSessionId}
                            onChange={(e) => setStartSessionId(e.target.value)}
                            disabled={isLoadingSessions || sessions.length === 0}
                            className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-purple-500 disabled:opacity-50"
                        >
                    ...
                    {/* End Session */}
                    <div>
                        <label className="block text-sm text-slate-400 mb-2">End Session (Later)</label>
                        <select
                            value={endSessionId}
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                            Variance = Actual Count − Expected Stock | <span className="text-red-400">Negative = Missing</span> | <span className="text-green-400">Positive = Surplus</span>
                        </p>
                    </div>
                </div>
            </div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                            ...
                            <p className="text-2xl font-bold text-white">{report.summary.totalItems}</p>
                        </div>
                        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                            <p className="text-slate-400 text-sm">Items with Variance</p>
                            <p className="text-2xl font-bold text-amber-400">{report.summary.itemsWithVariance}</p>
                        </div>
                        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
                            <div className="flex items-center gap-2">
                                <TrendingDown size={16} className="text-red-400" />
                                <p className="text-red-300 text-sm">Missing Stock Value</p>
                            </div>
                            <p className="text-2xl font-bold text-red-400">{formatCurrency(report.summary.totalMissingCost)}</p>
                        </div>
                        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
                            <div className="flex items-center gap-2">
                                <TrendingUp size={16} className="text-green-400" />
                                <p className="text-green-300 text-sm">Surplus Value</p>
                            </div>
                            <p className="text-2xl font-bold text-green-400">{formatCurrency(report.summary.totalSurplusCost)}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                        <div className="bg-slate-800/50 border border-slate-700 rounded-lg px-4 py-2">
                            ...
                            <span className="text-white">{formatDate(report.startSession.startedAt)}</span>
                        </div>
                        <div className="bg-slate-800/50 border border-slate-700 rounded-lg px-4 py-2">
                            <span className="text-slate-400">End: </span>
                            <span className="text-white">{formatDate(report.endSession.startedAt)}</span>
                        </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-slate-700 bg-slate-900/50">
                                        ...
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Starting</th>
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Purchased</th>
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Expected</th>
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Actual</th>
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Variance</th>
                                        <th className="text-right p-4 text-slate-400 font-medium text-sm">Variance Cost</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {report.items.length === 0 ? (
                                        ...
                                        report.items.map(item => (
                                            <tr
                                                key={item.itemId}
                                                className={`border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors ${getVarianceRowClass(item)}`}
                                            >
                                                <td className="p-4">
                    ...
                    {/* Net Variance */}
                    <div className={`p-6 rounded-xl border ${report.summary.totalVarianceCost < 0 ? 'bg-red-500/10 border-red-500/30' : 'bg-green-500/10 border-green-500/30'}`}>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-lg font-semibold text-white">Net Variance</span>
                            </div>
                            <span className={`text-3xl font-bold ${report.summary.totalVarianceCost < 0 ? 'text-red-400' : 'text-green-400'}`}>
                                {report.summary.totalVarianceCost < 0 ? '-' : '+'}
