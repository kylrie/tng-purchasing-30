...
import { Martini, UtensilsCrossed, Gamepad2, Plus, Sparkles, ChevronRight, Flame } from 'lucide-react';
...
/**
 * The Fun Roof — standalone QR menu (business unit b1 · "THE FUN GUYS CORP.").
 *
 * A public, POS-free customer menu built on the same QR product design language
 * as the Inflatable Island module, re-skinned into The Fun Roof's dark neon
 * rooftop-nightlife identity. It renders the owner's curated menu (read-only
 * snapshot) across Drinks · Food · Play, and every item without a photo shows a
 * clean fallback tile — never a broken image.
 *
 * Boundary: browse only. Picks are a LOCAL shortlist; nothing is submitted to the
 * POS, no order/payment/kitchen path is touched (see FunRoofCartDrawer).
 */
...
/** Neutral image-zone tile shown until an item has a real photo. Icon follows the
 *  group so image-less items still read intentional. */
...
const FunRoofMenuView: React.FC = () => {
    // Optional table param (e.g. /funroof/12). Falls back to a demo number so the
    // header always shows a table, matching the Inflatable Island demo.
    const { tableId } = useParams<{ tableId?: string }>();
    const tableNumber = (tableId && tableId.trim()) || '12';

    // The complete menu from the read-only snapshot (swap-point for a future live
    // b1 read lives in loadFunRoofMenu — the view never changes).
    ...
    const subcategories = useMemo(
        () => FUN_ROOF_MENU_GROUPS.find(g => g.key === activeGroup)?.subcategories ?? [],
        [activeGroup],
    );
    const visibleItems = useMemo(
        () => menuItems.filter(i => i.group === activeGroup && i.category === activeSub),
    ...
    return (
        <div className="min-h-dvh relative overflow-x-hidden" style={{ background: FUN_ROOF_BACKGROUND, color: T.text }}>
            <div className="relative z-10 max-w-md mx-auto w-full px-5">
                {/* Header hero: glowing logo + table number + picks button */}
                <header className="relative pt-9">
                    <div className="relative mx-auto w-full flex flex-col items-center">
                        <div className="relative">
                            ...
                            {/* Official logo, cropped to the wordmark + optimized (webp). Swap in a
                                new raster later at the same path with no layout change. */}
                            <img
                    ...
                    {/* Table number (matches the Inflatable Island placement; Fun Roof neon) */}
                    <div className="relative mt-4 h-16">
                        <div className="absolute inset-0 flex items-center justify-center text-center">
                            <div
                                className="text-[42px] leading-none font-extrabold tracking-tight max-w-[70vw] truncate px-2"
                                style={{
                                    backgroundImage: 'linear-gradient(92deg, #37D3E6 0%, #7FE0A8 52%, #A7E739 100%)',
                        ...
                        >
                            ...
                            {pickCount > 0 && (
                                <span key={pickCount} className="qr-pop absolute -top-1 -right-1 min-w-[22px] h-[22px] px-1.5 rounded-full text-[11px] font-bold flex items-center justify-center text-white" style={{ background: FUN_ROOF_COLORS.magenta }}>
                                    {pickCount}
                                </span>
                            )}
                        </button>
                    ...
                    {FUN_ROOF_MENU_GROUPS.map(group => {
                        ...
                        return (
                            <button
                                ...
                                style={isActive
                                    ? { background: FUN_ROOF_CTA_GRADIENT, color: '#fff', boxShadow: '0 10px 26px -8px rgba(245,32,155,0.6)' }
                    ...
                    {subcategories.map(sub => {
                        ...
                        return (
                            <button
                                ...
                                style={isActive
                                    ? { background: FUN_ROOF_COLORS.magenta, color: '#fff', boxShadow: '0 6px 16px rgba(245,32,155,0.4)' }
                                    : { background: T.surface, color: n === 0 ? T.textFaint : T.textMuted, border: `1px solid ${T.border}` }}
                            >
                                {sub}
                        ...
                        visibleItems.map(item => (
                            <div key={item.id} className="flex items-center gap-4 rounded-[20px] p-4" style={{ background: T.surface, border: `1px solid ${T.border}`, boxShadow: '0 10px 30px -12px rgba(0,0,0,0.6)' }}>
                                <button
                                    ...
                                    className="flex items-center gap-3.5 flex-1 min-w-0 text-left rounded-2xl outline-none focus-visible:ring-2"
                                    style={{ '--tw-ring-color': FUN_ROOF_COLORS.magenta } as React.CSSProperties}
                                >
                                    <div className="w-20 h-20 rounded-2xl overflow-hidden shrink-0" style={{ border: `1px solid ${T.border}` }}>
                                        {item.imageUrl ? (
                                            <img src={item.imageUrl} alt="" loading="lazy" decoding="async" width={80} height={80} className="w-full h-full object-cover" />
                                        ) : (
                                            <ImageFallback group={item.group} />
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0 py-0.5">
                                        <h3 className="font-bold text-[16px] leading-tight line-clamp-2" style={{ color: T.text }}>{item.name}</h3>
                                        {item.description && (
                                            <p className="text-[13px] leading-snug mt-1 line-clamp-2" style={{ color: T.textMuted }}>{item.description}</p>
                                        )}
                                        <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                                            ...
                                            {!item.bestSeller && item.tag && item.tag.length <= 16 && (
                                                <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ color: FUN_ROOF_COLORS.cyan, background: 'rgba(55,211,230,0.12)', border: `1px solid rgba(55,211,230,0.3)` }}>
                                                    {item.tag}
                                                </span>
                                            )}
                                            {item.serving && (
                                                <span className="text-[12px] font-medium" style={{ color: T.textFaint }}>{item.serving}</span>
                                            )}
                                        </div>
                                    </div>
                                </button>
                                <div className="flex flex-col items-end justify-between self-stretch py-1 shrink-0">
                                    ...
                                    <button
                                        ...
                                        aria-label={`Add ${item.name} to my picks`}
                                        className="w-[46px] h-[46px] rounded-2xl flex items-center justify-center active:scale-95 transition-transform duration-200"
                                        style={{ background: T.primarySoft, border: `1px solid ${T.borderStrong}` }}
                                    >
                ...
                <div className="fixed bottom-0 inset-x-0 z-40 px-4 pt-2 pb-[max(1rem,env(safe-area-inset-bottom))]">
                    <div className="max-w-md mx-auto">
                        <button
                            type="button"
                            onClick={() => setPicksOpen(true)}
                            className="w-full flex items-center gap-4 rounded-[24px] px-5 py-4 text-white active:scale-[0.97] transition-transform duration-200"
                            style={{ background: FUN_ROOF_CTA_GRADIENT, boxShadow: '0 16px 40px -10px rgba(245,32,155,0.6)' }}
                        >
                            <div className="relative shrink-0">
                                <Sparkles size={24} strokeWidth={2} />
                                <span key={pickCount} className="qr-pop absolute -top-2 -right-2 min-w-[22px] h-[22px] px-1.5 rounded-full text-[11px] font-bold flex items-center justify-center" style={{ background: '#fff', color: FUN_ROOF_COLORS.magenta }}>
                                    {pickCount}
                                </span>
                            </div>
            ...
            <FunRoofCartDrawer open={picksOpen} lines={picks} tableNumber={tableNumber} onClose={() => setPicksOpen(false)} onChangeQty={changeQty} onRemove={removePick} onClear={() => setPicks([])} />
        </div>
    );
};
