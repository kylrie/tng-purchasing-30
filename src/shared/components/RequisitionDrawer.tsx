                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-white">{requisition.id}</h2>
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 whitespace-normal break-words">{requisition.description || 'No description'}</p>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {activeTab === 'items' && (
                        <div className="space-y-6">
                            ...
                            {requisition.remarks && (
                                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                                    <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-300 mb-2 uppercase tracking-wider">Remarks</h3>
                                    <p className="text-slate-700 dark:text-white text-sm whitespace-pre-wrap">{requisition.remarks}</p>
                                </div>
                            )}
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tfoot className={`${colors.bgAccent} border-t ${colors.borderAccent}`}>
                                            <tr>
                                                ...
                                                <td className="px-4 py-3 text-right text-slate-900 dark:text-white font-bold text-lg">
                            ...
                            {requisition.prfDetails?.supplier ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                                                ...
                                                <p className="text-slate-900 dark:text-white font-medium">{requisition.prfDetails.supplier.tin || '-'}</p>
                                            </div>
                                            {/* Payment Mode */}
                                            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                                                <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Payment Mode</p>
                                                <p className="text-slate-900 dark:text-white font-medium">{requisition.prfDetails.supplier.paymentMode || '-'}</p>
                                            </div>
                                            {/* Terms */}
                                            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                                                <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Terms</p>
                                                <p className="text-slate-900 dark:text-white font-medium">{requisition.prfDetails.supplier.terms || '-'}</p>
                                            </div>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                                        ...
                                        <p className="text-slate-900 dark:text-white">{requisition.prfDetails.supplier.address || 'No address on file'}</p>
                                    </div>

                                    {/* Bank Details */}
                                    {requisition.prfDetails.supplier.bankDetails && (
                                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                                            <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-300 mb-3 uppercase tracking-wider">Bank Details</h4>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="bg-slate-100 dark:bg-slate-900/50 rounded-lg p-3">
                                                    ...
                                                    <p className="text-slate-900 dark:text-white font-medium">{requisition.prfDetails.supplier.bankDetails.bankName || '-'}</p>
                                                </div>
                                                <div className="bg-slate-100 dark:bg-slate-900/50 rounded-lg p-3">
                                                    <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Account Number</p>
                                                    <p className="text-slate-900 dark:text-white font-mono">{requisition.prfDetails.supplier.bankDetails.accountNumber || '-'}</p>
                                                </div>
                                                <div className="col-span-2 bg-slate-100 dark:bg-slate-900/50 rounded-lg p-3">
                                                    <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Account Name</p>
                                                    <p className="text-slate-900 dark:text-white font-medium">{requisition.prfDetails.supplier.bankDetails.accountName || '-'}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                        ...
                        <div>
                            ...
                            {((parentBurf?.history && parentBurf.history.length > 0) || (requisition.history && requisition.history.length > 0)) ? (
                                        ...
                                        {combinedHistoryEntries.map((item, index) => {
                                            ...
                                            return (
                                                <div key={`${source}-${entry.timestamp || entry.date}-${index}`} className="relative">
                                                    ...
                                                    <div className={`${isBurf ? 'bg-purple-900/20 border-purple-700/30' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700/50'} rounded-lg p-3 border`}>
                                                        <div className="flex items-center justify-between mb-1">
                                                            <span className={`text-sm font-medium ${isBurf ? 'text-purple-600 dark:text-purple-300' : 'text-slate-900 dark:text-white'}`}>{entry.action}</span>
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-slate-500 dark:text-slate-500">No history available</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'attachments' && (
                        <div className="space-y-4">
                            ...
                            {(requisition.chequeNumber || requisition.chequeImageUrl) && (
                                <div className="bg-gradient-to-br from-emerald-50 to-slate-50 dark:from-emerald-900/30 dark:to-slate-800/50 rounded-lg p-4 border border-emerald-200 dark:border-emerald-700/50">
                                    <h4 className="text-xs uppercase tracking-wider text-emerald-600 dark:text-emerald-400 mb-3 flex items-center gap-2">
                                        <CreditCard size={14} />
                                    ...
                                    <div className="space-y-3">
                                        {requisition.chequeNumber && (
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-slate-500 dark:text-slate-400">Cheque Number</span>
                                                <span className="text-slate-900 dark:text-white font-mono font-medium">{requisition.chequeNumber}</span>
                                            </div>
                                        )}
                                        {requisition.chequeImageUrl && (
                                            <a
                                                ...
                                                className="flex items-center gap-3 p-3 bg-white dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-emerald-500/50 transition-colors group"
                                            >
                                                <ExternalLink size={18} className="text-emerald-500 dark:text-emerald-400" />
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400">View Check Image</p>
                                ...
                                <div className="space-y-3">
                                    {requisition.externalLink && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-blue-500/50 transition-colors group"
                                        >
                                            <ExternalLink size={20} className="text-blue-500 dark:text-blue-400" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400">External Reference</p>
                                    ...
                                    {requisition.attachments?.map((url: string, index: number) => (
                                        <a
                                            ...
                                            className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-blue-500/50 transition-colors group"
                                        >
                                            <Paperclip size={20} className="text-slate-400 dark:text-slate-400" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400">Attachment {index + 1}</p>
